...
# The same test as in udp.t but using rejig commands instead.
...
sub udp_set_test {
    ...
    if ($protocol == ::IS_ASCII) {
        $req = "rj $config_id $fragment_id set $key $flags $exp $val_len\r\n$value\r\n";
...
sub udp_get_test {
    ...
    if ($protocol == ::IS_ASCII) {
        $req = "rj $config_id $fragment_id get $key\r\n";
    } elsif ($protocol == ::IS_BINARY) {
...
sub udp_delete_test {
    ...
    if ($protocol == ::IS_ASCII) {
        $req = "rj $config_id $fragment_id delete $key\r\n";
    } elsif ($protocol == ::IS_BINARY) {
...
sub udp_incr_decr_test {
    ...
    my $key_len = length($key);
    my $expected_value = 0;
    my $acmd = "rj $config_id $fragment_id incr";
    my $bcmd = ::CMD_INCR;
    if ($optype eq "incr") {
        $expected_value = $init_val + $val;
    } else {
        $acmd = "rj $config_id $fragment_id decr";
        $bcmd = ::CMD_DECR;
        $expected_value = $init_val - $val;
    }
