...
print $sock "rj $config_id $fragment_id add bar 0 0 6\r\nbarval\r\n";
is(scalar <$sock>, "STORED\r\n", "stored barval");
rejig_mem_get_is($sock, $config_id, $fragment_id, "bar", "barval");

# add foo (but shouldn't get new value)
print $sock "rj $config_id $fragment_id add foo 0 0 5\r\nfoov2\r\n";
is(scalar <$sock>, "NOT_STORED\r\n", "not stored");
rejig_mem_get_is($sock, $config_id, $fragment_id, "foo", "fooval");

# replace bar (should work)
print $sock "rj $config_id $fragment_id replace bar 0 0 6\r\nbarva2\r\n";
is(scalar <$sock>, "STORED\r\n", "replaced barval 2");

# replace notexist (shouldn't work)
print $sock "rj $config_id $fragment_id replace notexist 0 0 6\r\nbarva2\r\n";
is(scalar <$sock>, "NOT_STORED\r\n", "didn't replace notexist");

# delete foo.
print $sock "rj $config_id $fragment_id delete foo\r\n";
...
# add moo
print $sock "rj $config_id $fragment_id add moo 0 0 6\r\nmooval\r\n";
is(scalar <$sock>, "STORED\r\n", "stored barval");
rejig_mem_get_is($sock, $config_id, $fragment_id, "moo", "mooval");

# check-and-set (cas) failure case, try to set value with incorrect cas unique val
print $sock "rj $config_id $fragment_id cas moo 0 0 6 0\r\nMOOVAL\r\n";
is(scalar <$sock>, "EXISTS\r\n", "check and set with invalid id");

# test "gets", grab unique ID
print $sock "rj $config_id $fragment_id gets moo\r\n";
...
print $sock "rj $config_id $fragment_id cas moo 0 0 6 $unique_id\r\nMOOVAL\r\n";
is(scalar <$sock>, "STORED\r\n");
rejig_mem_get_is($sock, $config_id, $fragment_id, "moo", "MOOVAL");

# pipeline is okay
print $sock "rj $config_id $fragment_id set foo 0 0 6\r\nfooval\r\nrj $config_id $fragment_id delete foo\r\nrj $config_id $fragment_id set foo 0 0 6\r\nfooval\r\nrj $config_id $fragment_id delete foo\r\n";
is(scalar <$sock>, "STORED\r\n",  "pipeline set");
is(scalar <$sock>, "DELETED\r\n", "pipeline delete");
is(scalar <$sock>, "STORED\r\n",  "pipeline set");
is(scalar <$sock>, "DELETED\r\n", "pipeline delete");
...
rejig_mem_get_is($sock, $config_id, $fragment_id, "foo", "fooval");
$config_id = 2;
print $sock "rj $config_id $fragment_id set bar 0 0 6\r\nbarval\r\n";
is(scalar <$sock>, "STORED\r\n", "stored bar");
rejig_mem_get_is($sock, $config_id, $fragment_id, "bar", "barval");
print $sock "rj $config_id $fragment_id get foo bar\r\n";
...
print $sock "rj $config_id $fragment_id get foo bar foo bar foo bar foo bar foo bar foo bar foo bar foo bar foo bar foo bar foo bar foo bar foo bar foo bar foo bar foo bar foo bar foo bar foo bar foo bar\r\n";
my $i = 0;
while ($i < 20) {
    is(scalar <$sock>, "VALUE foo 0 6 1\r\n");
    is(scalar <$sock>, "fooval\r\n");
...
while ($len < 1024*1028) {
    ...
    if ($len > (1024*1024)) {
        ...
        print $sock "rj $config_id $fragment_id set foo_$len 0 0 3\r\nMOO\r\n";
        is(scalar <$sock>, "STORED\r\n");
        print $sock "rj $config_id $fragment_id set foo_$len 0 0 $len\r\n$val\r\n";
        is(scalar <$sock>, "SERVER_ERROR object too large for cache\r\n", "failed to store size $len");
        rejig_mem_get_is($sock, $config_id, $fragment_id, "foo_$len");
    } else {
        print $sock "rj $config_id $fragment_id set foo_$len 0 0 $len\r\n$val\r\n";
        is(scalar <$sock>, "STORED\r\n", "stored size $len");
    }
